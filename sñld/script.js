...
/* Restart button wiring with mobile hide/reveal behavior */
const restartBtn = document.getElementById('restart-btn');
const restartReveal = document.getElementById('restart-reveal');

function showReveal() {
  if (!restartReveal) return;
  restartReveal.classList.remove('hidden');
  restartReveal.style.display = 'inline-flex';
  restartReveal.hidden = false;
  restartReveal.setAttribute('aria-hidden', 'false');
}
function hideReveal() {
  if (!restartReveal) return;
  restartReveal.classList.add('hidden');
  restartReveal.style.display = 'none';
  restartReveal.hidden = true;
  restartReveal.setAttribute('aria-hidden', 'true');
}
// Instead of hiding the restart button (which caused it to become unreachable in fullscreen/BIOS),
// simply disable it while BIOS or other modal actions are active so the button remains visible
// and available to the user when needed.
function hideRestartBtn() {
  if (!restartBtn) return;
  restartBtn.disabled = true;
  restartBtn.setAttribute('aria-hidden', 'true');
  // keep the button visible (do not remove from layout) so users can always see/press it when enabled
  restartBtn.style.opacity = '0.6';
  restartBtn.style.pointerEvents = 'auto';
}
// When showing the restart button, ensure it is enabled and visible.
function showRestartBtn() {
  if (!restartBtn) return;
  restartBtn.disabled = false;
  restartBtn.removeAttribute('aria-hidden');
  restartBtn.style.display = 'inline-flex';
  restartBtn.style.opacity = '0.95';
  restartBtn.style.pointerEvents = 'auto';
}

if (restartBtn) {
  // central restart action used by non-touch restart button and other triggers
  async function doRestart() {
    // visually disable while restarting to avoid double clicks
    restartBtn.disabled = true;
    // clear any running boot interval
    if (bootIntervalHandle) { clearInterval(bootIntervalHandle); bootIntervalHandle = null; }
    // hide BIOS if visible, reset flags
    hide(biosEl);
    biosEl.setAttribute('aria-hidden','true');
    skipSequence = false;

    // If something is fullscreen, exit it first so the UI is available and clicks are effective.
    try {
      if (document.fullscreenElement) {
    ...
    // small delay to allow UI to update, then restart sequence
    setTimeout(()=> {
      startSequence();
      // re-enable after a short while
      setTimeout(()=> { restartBtn.disabled = false; }, 800);
    }, 120);
  }

  restartBtn.onclick = () => {
    // On touch devices: perform the same immediate restart as non-touch devices
    // (previous behavior hid the restart button and showed a reveal control; now touch triggers restart directly)
    doRestart();
  };

  // expose for other handlers if needed
  restartBtn.doRestart = doRestart;
}

// Reveal control: restore restart button or trigger an immediate restart on mobile
if (restartReveal) {
  restartReveal.onclick = () => {
    // If user taps the reveal, hide the reveal UI and show the restart control briefly.
    // Also perform an immediate restart so mobile users can restart without another tap.
    hideReveal();
    showRestartBtn();
    // if a restart action is available, run it (this triggers the same flow as desktop restart)
    if (restartBtn && typeof restartBtn.doRestart === 'function') {
      restartBtn.doRestart();
    }
  };
}
